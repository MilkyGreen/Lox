 ...
 * 根据key的hash值确定数组索引，每个索引只存一个entry。如果遇到冲突，则继续向期望索引的后面遍历，直到遇到的第一个空位置或者墓碑节点。
 * 删除key时，并不真的把相应位置的entry置为空，而是标记为墓碑节点（不这么做的话无法保证get方法的正确性）。墓碑节点可以被后面的set方法复用。此时count数量不减少，不然可能会出现全都是墓碑count=0的情况，findEntry永远在找非墓碑空节点。
